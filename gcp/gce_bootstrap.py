...
def main():
    ...
    site_git = input("site git: ")
    caddy_domain_name = input("domain name: ")
    git_hook_key = input("git hook key: ")
    caddy_env = (
        'Environment=CADDY_DOMAIN_NAME=%s\n'
        'Environment=CADDY_GIT_REPO=%s\n'
        'Environment=CADDY_GIT_HOOK_KEY=%s\n'
        'Environment=CLOUDFLARE_EMAIL=%s\n'
        'Environment=CLOUDFLARE_API_KEY=%s\n'
    ) % (caddy_domain_name, site_git, git_hook_key, email, cloudflare_key)
    env = 'CADDY_EMAIL="%s" CADDY_ENV="%s"' % (email, caddy_env)
    os.system(env+" sh ./gcp/gce_bootstrap.sh")
